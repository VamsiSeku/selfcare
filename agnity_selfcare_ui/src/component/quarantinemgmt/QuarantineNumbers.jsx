...
class QuarantineNumbers extends React.Component
{
    constructor(props)
    {
        super(props);
        this.state = {
            ...
            pageViewChosen : 'Quarantine Numbers',
    ...
    loadQuarantineNumbers(inOffset)
    {
        let self = this;
        ...
        $.ajax({
            method: "GET",
            url: this.state.param.RANI_API_INTEGRATION + SPSConstants.API_BASE_URL + "service/rani/account/" + encodeURIComponent(accountNumber) + "/quarantinelist?offset=" + inOffset + "&filter=" + encodeURIComponent(JSON.stringify(filter)),
            dataType: 'json',
    ...
    onPageChanged(data)
    {
        ...
        this.setState({ currentPage, totalPages, pageInfo }, () => {
            this.loadQuarantineNumbers(new_offset);
        });
    }
    ...
    render(){
        ...
        return(
            <I18nProvider locale={this.state.param.languageselected}>
                <div className="QuarantineMgmtHolder">
                    <div className="card QuarantineMgmtPanel SPSComponentPanel">
                        <div className="ScreenNavigationInfoPanel">
                            <BS.Breadcrumb className="ScreenNavigationListArea">
                                ...
                                <BS.BreadcrumbItem>
                                    <span>{Translator('quarantine_mgmt')}</span>
                                </BS.BreadcrumbItem>
                        ...
                        <div className="FeatureSelectionPanel">
                            <BS.Tabs variant="tabs" style={{background: '#d1dff8'}} activeKey={this.state.pageViewChosen} 
                                ...
                                <BS.Tab eventKey="Quarantine Numbers" title={Translator('quarantine_numbers')}>
                    ...
                    <div className="SPSComponentTableHolder" style={{ marginTop: '1%' }}>
                        <BS.Table className="SPSComponentTable table-striped">
                            <thead className="SPSComponentTblHeader">
                                <tr>
                                    <th>{Translator('quarantine_aninumber')}</th>
                                    <th>{Translator('quarantine_date')}</th>
                                    <th>{Translator('quarantine_status')}</th>
                            ...
                            <tbody className="SPSComponentTblBody">
                                {
                                    this.state.records.map((item, key) => 
                                        <tr key={item.aniNumber}>
                                            ...
                                            <td>{SPSUtils.getNumberPoolStatus(2)}</td>
                                            <td>
                                                <div className="row ActionIcons">
                                                    
                                                    <button className="d-flex p-1 btn btn-danger btn-transparent"
                                                        data-toggle="tooltip"
                                                        data-placement="top" disabled={this.state.userRole.adminRole === '2'}
                                                        title="Move To Free" onClick={() => this.deAssignFromCampaign(item)}
                                                    >
...
export default QuarantineNumbers;